...
function App() {
  ...
  }, [theme, accent, reducedMotion, systemReducedMotion])
  ...
  const stats = useMemo(() => ({ total: cards.length, skills: cards.filter(c => c.type === '技能').length, notes: cards.filter(c => c.type === '知识').length, favorites: cards.filter(c => c.favorite).length }), [cards])
  ...
  const createCard = async (card: Card) => {
    try {
      ...
      await loadCards(); setShowComposer(false); showNotice('已新增卡片并保存到本地 Markdown + SQLite')
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${mobileNav ? 'mobile-open' : ''}`}>
      <div className="brand"><div className="brand-mark">✦</div><div><strong>我的工作台</strong><span>PERSONAL AI DESK</span></div><button className="mobile-close" onClick={() => setMobileNav(false)}><X size={18} /></button></div>
      <div className="workspace-pill"><div className="avatar">H</div><div><b>Hermes Workspace</b><small>本地优先 · 私人</small></div><ChevronRight size={15} /></div>
      <nav className="nav-list">{nav.map(({ id, icon: Icon }) => <button key={id} className={`nav-item ${active === id ? 'active' : ''}`} onClick={() => { setActive(id); setMobileNav(false) }}><Icon size={17} /><span>{id}</span>{id === '全部内容' && <em>{stats.total}</em>}</button>)}</nav>
      ...
      <div className="sidebar-bottom"><div className="storage-status"><span className="status-dot" /> <span>本地存储正常</span><small>SQLite · Markdown</small></div><div className="profile-row"><div className="profile-avatar">H</div><div><b>我的个人空间</b><small>仅本机可见</small></div><SlidersHorizontal size={16} /></div></div>
    </aside>
    ...
    <main className="main-content">
      ...
      <section className="hero museum-hero" onMouseMove={event => { if (reducedMotion || systemReducedMotion) return; const rect = event.currentTarget.getBoundingClientRect(); event.currentTarget.style.setProperty('--hero-shift-x', `${((event.clientX - rect.left) / rect.width - .5) * 10}px`); event.currentTarget.style.setProperty('--hero-shift-y', `${((event.clientY - rect.top) / rect.height - .5) * 6}px`) }} style={active === '总览' ? { '--museum-hero': `url(${museumHero})` } as React.CSSProperties : undefined}>{active === '总览' && atmosphereReady && <Suspense fallback={<div className="museum-atmosphere-fallback" aria-hidden="true" />}><MuseumAtmosphere reducedMotion={reducedMotion || systemReducedMotion} theme={theme} /></Suspense>}<div className="museum-hero-content"><div className="eyebrow"><span className="eyebrow-dot" /> EASTERN DIGITAL LIBRARY</div><h1>{active === '总览' ? <>把重要的东西，<i>放在一起。</i></> : active}</h1><p>{active === '总览' ? '在一座明亮的东方数字图书馆里，浏览、收藏和继续使用你的知识资产。' : `在 ${active} 中找到你需要的内容。`}</p></div><div className="hero-space-grid" aria-hidden="true" /><div className="hero-light-column light-column-a" aria-hidden="true" /><div className="hero-light-column light-column-b" aria-hidden="true" /><div className="hero-orbit"><div className="orbit-ring ring-one" /><div className="orbit-ring ring-two" /><div className="orbit-ring ring-three" /><div className="orbit-core">✦</div></div></section>
      {active === '设置' ? <SettingsView onNotice={showNotice} /> : <><section className="toolbar"><div className="search-box"><Search size={18} /><input value={searchDraft} onChange={e => setSearchDraft(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') executeSearch() }} placeholder="搜索工作台中的内容..." />{searchDraft && <button aria-label="清空搜索" onClick={() => { setSearchDraft(''); setQuery('') }}><X size={15} /></button>}<button className="search-submit" aria-label="查询内容" onClick={executeSearch}>查询</button><kbd>⌘ K</kbd></div><div className="filter-row"><div className="select-wrap"><Tag size={15} /><select value={tagFilter} onChange={e => setTagFilter(e.target.value)}>{allTags.map(tag => <option key={tag}>{tag}</option>)}</select></div><div className="select-wrap"><select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}>{allStatuses.map(status => <option key={status}>{status}</option>)}</select></div><span className="result-count">找到 {filtered.length} 张卡片</span></div></section>
...
function StateCard({ kind, onAction }: { kind: 'loading' | 'error' | 'empty' | 'no-content'; onAction: () => void }) { const states = { loading: { icon: <RefreshCw className="spin" size={28} />, title: '正在打开东方数字图书馆', text: '正在从本机 SQLite 与 Markdown 读取展品。', action: '重新读取' }, error: { icon: <ShieldCheck size={28} />, title: '展厅暂时无法打开', text: '本地数据服务没有返回有效内容。数据没有被修改。', action: '重试' }, empty: { icon: <Search size={28} />, title: '没有找到对应展品', text: '可以调整关键词、标签或状态筛选。', action: '清除筛选' }, 'no-content': { icon: <Sparkles size={28} />, title: '展厅还没有展品', text: '可以新建一张卡片，或导入本地文档开始收藏。', action: '新建/导入' } }[kind]; return <div className={`empty-state state-${kind}`}><div className="empty-icon">{states.icon}</div><h3>{states.title}</h3><p>{states.text}</p><button className="quiet-button" onClick={onAction}>{states.action}</button></div> }
...
function SettingsView({ onNotice }: { onNotice: (message: string) => void }) {
  ...
  return <section className="settings-view">
    <section className="settings-card"><div className="settings-card-head"><span className="eyebrow">DATA SAFETY</span><h2>本地数据安全</h2></div><p className="muted-copy">所有卡片、版本、关系和原始副本都只保存在本机。备份会把 SQLite、Markdown 和原始文件打包为 ZIP。</p><div className="settings-actions"><button className="quiet-button" onClick={() => void runBackup()} disabled={backupState === 'running'}>{backupState === 'running' ? '正在备份…' : '立即创建 ZIP 备份'}</button>{backupState === 'done' && <span className="settings-ok">备份完成</span>}{backupState === 'error' && <span className="settings-warn">备份失败</span>}</div></section>
